...
# Add src to path so we can import config_loader
...
def _sub_build_corpus(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--raw-dir",    default=get_config_value(_CFG, "data.raw_dir", "data/retrieval_corpus/raw"),
...
def _sub_build_qa(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--corpus",     default=get_config_value(_CFG, "data.corpus_ar", "data/retrieval_corpus/processed/DHDA_filtered_AR.csv"),
...
def _sub_build_qd(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--corpus",      default=get_config_value(_CFG, "data.corpus_ar", "data/retrieval_corpus/processed/DHDA_filtered_AR.csv"))
    p.add_argument("--output",      default=get_config_value(_CFG, "data.qd_full", "data/questions_data/QD_data/QD_full_data.csv"))
    p.add_argument("--eval-output", default=None,
                   help="Path for the evaluation split (default: auto-derived).")
    p.add_argument("--n-sample",    type=int, default=get_config_value(_CFG, "data_generation.qd_n_sample", 1002),
...
def _sub_build_ce_data(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--qd-path",       default=get_config_value(_CFG, "data.qd_full", "data/questions_data/QD_data/QD_full_data.csv"))
    p.add_argument("--output",        default=get_config_value(_CFG, "data.qd_contrastive", "data/questions_data/QD_data/QD_contrastive_data.csv"))
    p.add_argument("--n-source",      type=int, default=get_config_value(_CFG, "cross_encoder_training.n_source", 5000))
    p.add_argument("--num-negatives", type=int, default=get_config_value(_CFG, "cross_encoder_training.num_negatives", 1))
    p.add_argument("--random-state",  type=int, default=get_config_value(_CFG, "random_seed", 42))
...
def _sub_build_clf_data(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--corpus",       default=get_config_value(_CFG, "data.corpus_ar", "data/retrieval_corpus/processed/DHDA_filtered_AR.csv"))
    p.add_argument("--inscriptions", default=get_config_value(_CFG, "data.inscriptions", "data/retrieval_corpus/processed/DHDA_inscriptions_data.csv"))
    p.add_argument("--output",       default=get_config_value(_CFG, "data.clf_data", "data/questions_data/classification_data/classification_data.csv"))
    p.add_argument("--n-per-type",   type=int, default=get_config_value(_CFG, "data_generation.clf_n_per_type", 1000))
    p.add_argument("--random-state", type=int, default=get_config_value(_CFG, "random_seed", 42))
...
def _sub_train_classifier(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--data",         default=get_config_value(_CFG, "data.clf_data", "data/questions_data/classification_data/classification_data.csv"))
    p.add_argument("--output",       default=get_config_value(_CFG, "models.classifier", "models/RF_intent_classifier.joblib"))
    p.add_argument("--n-estimators", type=int,   default=get_config_value(_CFG, "classifier_training.n_estimators", 200),    dest="n_estimators")
    p.add_argument("--ngram-range",  type=int,   default=get_config_value(_CFG, "classifier_training.ngram_range", [3, 5]), nargs=2, dest="ngram_range")
    p.add_argument("--max-features", type=int,   default=get_config_value(_CFG, "classifier_training.max_features", 30_000), dest="max_features")
    p.add_argument("--test-size",    type=float, default=get_config_value(_CFG, "classifier_training.test_size", 0.2),    dest="test_size")
    p.add_argument("--threshold",    type=float, default=get_config_value(_CFG, "classifier_training.threshold", 0.6))
    p.add_argument("--random-state", type=int,   default=get_config_value(_CFG, "random_seed", 42),     dest="random_state")
...
def _sub_finetune_reranker(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--data",         default=get_config_value(_CFG, "data.qd_contrastive", "data/questions_data/QD_data/QD_contrastive_data.csv"))
    p.add_argument("--output",       default=get_config_value(_CFG, "models.cross_encoder", "models/finetuned_CE_bge"))
    p.add_argument("--model-name",   default=get_config_value(_CFG, "cross_encoder_training.base_model", "BAAI/bge-reranker-v2-m3"),  dest="model_name")
    p.add_argument("--max-length",   type=int,   default=get_config_value(_CFG, "cross_encoder_training.max_length", 512),   dest="max_length")
    p.add_argument("--epochs",       type=int,   default=get_config_value(_CFG, "cross_encoder_training.epochs", 3))
    p.add_argument("--batch-size",   type=int,   default=get_config_value(_CFG, "cross_encoder_training.batch_size", 8),     dest="batch_size")
    p.add_argument("--lr",           type=float, default=get_config_value(_CFG, "cross_encoder_training.learning_rate", 5e-5))
    p.add_argument("--warmup-steps", type=int,   default=get_config_value(_CFG, "cross_encoder_training.warmup_steps", 0),     dest="warmup_steps")
    p.add_argument("--eval-steps",   type=int,   default=get_config_value(_CFG, "cross_encoder_training.eval_steps", 500),   dest="eval_steps")
    p.add_argument("--amp",      action=argparse.BooleanOptionalAction, default=get_config_value(_CFG, "cross_encoder_training.use_amp", True),
                   dest="use_amp",
                   help="Use mixed-precision (fp16) training. Pass --no-amp to disable.")
    p.add_argument("--seed",         type=int,   default=get_config_value(_CFG, "random_seed", 42))
    p.add_argument("--val-ratio",    type=float, default=get_config_value(_CFG, "cross_encoder_training.val_ratio", 0.1),   dest="val_ratio")
    p.add_argument("--test-ratio",   type=float, default=get_config_value(_CFG, "cross_encoder_training.test_ratio", 0.1),   dest="test_ratio")
...
def _sub_build_index(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--corpus",     default=get_config_value(_CFG, "data.text_to_embed", "data/retrieval_corpus/processed/DHDA_text_to_embed.csv"),
    ...
    p.add_argument("--index-type", default=get_config_value(_CFG, "index.index_type", "Flat"), choices=["Flat", "IVFFlat", "HNSW"],
                   dest="index_type")
    p.add_argument("--batch-size", type=int, default=get_config_value(_CFG, "index.batch_size", 256), dest="batch_size")
...
def _sub_eval_retrieval(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--method",          default=get_config_value(_CFG, "retrieval.default_method", "hybrid"), choices=["bm25", "dense", "hybrid"])
    p.add_argument("--eval-data",       default=get_config_value(_CFG, "data.qd_eval", "data/questions_data/QD_data/QD_evaluation_data.csv"),
                   dest="eval_data")
    p.add_argument("--corpus",          default=get_config_value(_CFG, "data.corpus_ar", "data/retrieval_corpus/processed/DHDA_filtered_AR.csv"))
    p.add_argument("--text-data",       default=get_config_value(_CFG, "data.text_to_embed", "data/retrieval_corpus/processed/DHDA_text_to_embed.csv"),
                   dest="text_data")
    p.add_argument("--embeddings",      default=get_config_value(_CFG, "data.embeddings", "data/retrieval_corpus/vector_database/embeddings_nomic-embed.npy"))
    p.add_argument("--index",           default=get_config_value(_CFG, "data.faiss_index", "data/retrieval_corpus/vector_database/faiss_nomic-embed.index"))
    p.add_argument("--embedding-model", default=get_config_value(_CFG, "models.embedding_model", "models/nomic"), dest="embedding_model")
    p.add_argument("--classifier",      default=get_config_value(_CFG, "models.classifier", "models/RF_intent_classifier.joblib"))
    p.add_argument("--cross-encoder",   default=get_config_value(_CFG, "models.cross_encoder", "models/finetuned_CE_bge"), dest="cross_encoder",
                   help="Pass 'none' to disable reranking.")
    p.add_argument("--output",          default=None,
                   help="Output CSV. Defaults to retrieval_output/<method>_results.csv.")
    p.add_argument("--top-k",    type=int, default=get_config_value(_CFG, "retrieval.top_k", 10),  dest="top_k")
    p.add_argument("--k-bm25",   type=int, default=get_config_value(_CFG, "retrieval.k_bm25", 50),  dest="k_bm25")
    p.add_argument("--k-rrf",    type=int, default=get_config_value(_CFG, "retrieval.k_rrf", 300), dest="k_rrf")
    p.add_argument("--k-rerank", type=int, default=get_config_value(_CFG, "retrieval.k_rerank", 50),  dest="k_rerank")
...
def _sub_generate(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--input",    default=get_config_value(_CFG, "data.qa_full_testing", "data/questions_data/QA_data/evaluation_data/islamic_full_testing.csv"),
...
def _sub_judge(sub: argparse._SubParsersAction) -> None:
    ...
    p.add_argument("--api-delay",  type=float, default=get_config_value(_CFG, "judge.api_delay", 1.0), dest="api_delay",
